...
const ANIMATION_TIMER = isMobile ? 200 : 1000;
const KEY_UP = 38;
const KEY_DOWN = 40;
...
export default class ReactPageScroller extends React.Component {
    ...
    static defaultProps = {
        animationTimer: isMobile ? 700 :1000,
    ...
    goToPage = (number) => {
        ...
        if (!_.isEqual(componentIndex, number)) {
            if (!_.isNil(this["container_" + (number)]) && !this[scrolling]) {
                ...
                this._pageContainer.style.transform = `translate3d(0, ${(number) * -100}vh, 0)`;
    ...
    [scrollWindowUp] = () => {
        if (!this[scrolling] && !this.props.blockScrollUp) {
            if (!_.isNil(this["container_" + (this.state.componentIndex - 1)])) {
                ...
                this._pageContainer.style.transform = `translate3d(0, ${(this.state.componentIndex - 1) * -100}vh, 0)`;
    ...
    [scrollWindowDown] = () => {
        if (!this[scrolling] && !this.props.blockScrollDown) {
            if (!_.isNil(this["container_" + (this.state.componentIndex + 1)])) {
                this[disableScroll]()
                this[scrolling] = true;
                if(this.state.componentIndex==6)
                    if(isIOS)
                        this._pageContainer.style.transform = `translate3d(0, ${(6.4) * -100}vh, 0)`;
                    else this._pageContainer.style.transform = `translate3d(0, ${(6.3) * -100}vh, 0)`;
                else
                     this._pageContainer.style.transform = `translate3d(0, ${(this.state.componentIndex + 1) * -100}vh, 0)`;
